                        ...
                        <div class="employee-modal-input-div">
                            <div class="row">
                                ...
                                <div class="col-sm-6 employee-input-col">
                                    ...
                                    <span class="text-danger" style="font-size: 8px;width:200px;display:inline-block"
...
<div class="employee-div">
    <div class="employee-header">
        ...
        <div style="cursor: pointer" class="employee-header-right" data-bs-toggle="modal"
    ...
    <div class="employee-main-content">
        @php
            if (Auth::guard('web')->check()) {
                ...
                $employee = \App\Models\BusinessUser::where('user_id', Auth::guard('businessUser')->user()->user_id)
                    ->where('id', '!=', Auth::guard('businessUser')->user()->id)
                    ->get();
            }
        ...
        @foreach ($employee as $employee)
            <div class="single-employee-div">
                ...
                <div class="action-employee-div display-toggle"
                    style="text-align:center;width:100px;position: absolute; right:20px;top:10px;padding:10px;box-shadow: rgba(50, 50, 93, 0.25) 0px 2px 5px -1px, rgba(0, 0, 0, 0.3) 0px 1px 3px -1px;border-radius:5px">
                    <div style="cursor: pointer"
                        onclick="openEditModalEmployee('<?php echo $employee->id; ?>','<?php echo $employee->first_name; ?>','<?php echo $employee->last_name; ?>','<?php echo $employee->user_name; ?>','<?php echo $employee->email; ?>','<?php echo $employee->password; ?>','<?php echo $employee->phone; ?>','<?php echo $employee->designation; ?>','<?php echo $employee->status; ?>')">
                        Update
                    </div>
                    <div style="width: 100%;height:1px;background:gray;margin: 5px 0px"></div>
                    <div style="cursor: pointer">
                        <a style="text-decoration: none ; color:initial"
                            href="{{ route('user.deleteEmployee', $employee->id) }}">Delete</a>
                    </div>
                </div>
                <div class="image-div">
                    <div>
                        <img class="employee-image"
                            src="{{ $employee->img ? asset('image/user/companyUser/' . $employee->img) : asset('image/Photo.svg') }}"
                            alt="" />
                    </div>
                    @php
                ...
                <div class="info-div mt-3">
                    ...
                    <span>Trade Name: </span> <span>{{ $employee->business->business_profile->trade_name }}</span>
                    <br />
                    <span- style="word-break: break-all">Email: </span><span>{{ $employee->email }}</span> <br />
                        <span>Phone: </span><span>{{ $employee->phone }}</span> <br />
                        <span>End Date:
                        </span><span>{{ date('d/m/Y', strtotime($employee->business->business_profile->expiry_date)) }}</span>
                </div>
            </div>
        @endforeach
