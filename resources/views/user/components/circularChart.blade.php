@php
    ...
    $quotationsWithInvoiceSum = quote::leftJoin('invoices', 'quotes.id', '=', 'invoices.quote_id')
        ...
        ->selectRaw(
            'DATE_FORMAT(quotes.created_at, "%M") as month, COUNT(quotes.id) as count, SUM(invoices.total_price) as total',
        )
        ->groupBy('month')
        ->orderBy('month')
        ->get()
...
<div class="donut-chart-div">
    <div class="donut-chart-parent">
            ...
            <div class="single-chart">
                <svg viewBox="0 0 36 36" class="circular-chart green">
                    ...
                    <path class="circle" stroke-dasharray="30, 100" d="M18 2.0845
                            ...
                            a 15.9155 15.9155 0 0 1 0 -31.831" />
                    <text x="18" y="20.35"
                        class="percentage">{{ !empty($quotationsWithInvoiceSum) ? rtrim(rtrim(number_format($quotationsWithInvoiceSum[0]['total'], 2), '0'), '.') : 0 }}</text>
                </svg>
            </div>
        </div>
        <div class="donut-chart-text">
            ...
            <div class="chart-text-number">
                {{ !empty($quotationsWithInvoiceSum) ? rtrim(rtrim(number_format($quotationsWithInvoiceSum[0]['count'], 2), '0'), '.') : 0 }}
            </div>
