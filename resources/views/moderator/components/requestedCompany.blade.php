...
<div class="table-value hidden-common">
    ...
    <div class="name table-single-value">
        <a style="cursor: pointer; text-decoration: underline; color: blue"
            href="{{ route('moderator.requested.company', $item->id) }}">
            <span>{{ $item->business_profile['business_name'] }}</span> </a>
    </div>
    ...
    <div class="action table-single-value">
        <a href="{{ route('moderator.company.decline', $item->business_profile->id) }}"
            style="text-decoration: none; color:inherit"><button class="btn btn-outline-danger">Decline</button></a>
        <a href="{{ route('moderator.company.approve', $item->business_profile->id) }}"
            style="text-decoration: none; color:inherit"> <button class="btn btn-outline-success">Approve</button></a>
    </div>
</div>
