  ...
  <div class="table-value">
            ...
            <div class="status table-single-value" id="status-change-div" onclick="jobStatusChange('<?php echo $item->id; ?>','<?php echo $item->job_status[count($item->job_status)-1]->status; ?>')">
              <div class="status-div"  style="width: 100%; cursor: pointer;background:{{$arrBackground[$item->job_status[count($item->job_status)-1]->status]}};border:{{$arrBorder[$item->job_status[count($item->job_status)-1]->status]}}">
              
                <span style="font-weight: 500;color:{{$arrColor[$item->job_status[count($item->job_status)-1]->status]}}">{{$item->job_status[count($item->job_status)-1]->status}}</span>
            ...
            <div class="action table-single-value">
              <i
                class="fa-solid fa-ellipsis-vertical action-main-button-to-click"
              ></i>
              <div class="action-btn-div">
                <div class="action-btn-element">
                  <div class="edit">View Quotes</div>
                  <div class="delete">Invoice</div>
                  <div class="delete">Notes</div>
                  <div class="delete">Email Opened</div>
                  <div class="delete">Hide</div>
                  <div class="delete">Work From</div>
                </div>
              </div>
            </div>
 </div>
